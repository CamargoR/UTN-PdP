...
jugador(messi,argentina,delantero,0).
jugador(mess,argentina,delantero,0).
jugador(mes,argentina,delantero,0).
jugador(ronaldo,brasil,delantero,1).
jugador(ronalasddo,brasil,delantero,1).
jugador(ronald,brasil,delantero,1).
jugador(ronaldi,brasil,arquero,0).
jugador(asd,alemania,delantero,1).

esArquero(Jugador):-
	jugador(Jugador,_,arquero,_).
tieneAmarilla(Jugador):-
	jugador(Jugador,_,_,X),
	X>0.

perteneceA(Equipo,Jugador):-
	jugador(Jugador,Equipo,_,_).

esEquipo(Equipo):-jugador(_,Equipo,_,_).

esPeligroso(Equipo):-
	forall((jugador(Jugador,Equipo,_,_),not(esArquero(Jugador))),tieneAmarilla(Jugador)).
